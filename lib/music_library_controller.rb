...
def call 
    ...
    when "play song"
        ...
        "I'm sorry user, I'm afraid I can't do that right now"
    end 
end
end
...
def list_artists
    Artist.all.sort {|a,b| a.name <=> b.name}.each.with_index(1) {|artist, index|
puts "#{index}. #{artist.name}"}
end

def list_genres
    Genre.all.sort {|a,b| a.name <=> b.name}.each.with_index(1) {|genre, index|
